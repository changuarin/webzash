...
class Accountlist
{
	...
	function account_st_short($c = 0)
	{
		...
		if ($this->id != 0)
		{
			...
			echo "<td class=\"td-group\"nowrap>";
			echo $this->print_space($this->counter);
			echo "&nbsp;" .  $this->name;
			echo "</td>";
			echo "<td align=\"right\"nowrap>" . convert_amount_dc($this->total) . $this->print_space($this->counter) . "</td>";
			echo "</tr>";
		}
		...
		if (count($this->children_ledgers) > 0)
		{
			...
			foreach ($this->children_ledgers as $id => $data)
			{
				...
				echo "<td class=\"td-ledger\"nowrap>";
				echo $this->print_space($this->counter);
				echo "&nbsp;" . anchor('report/ledgerst/' . $data['id'], $data['name'], array('title' => $data['name'] . ' Ledger Statement', 'style' => 'color:#000000'));
				echo "</td>";
				echo "<td align=\"right\"nowrap>" . convert_amount_dc($data['total']) . $this->print_space($this->counter) . "</td>";
				echo "</tr>";
			}
			$this->counter--;
		}
	...
	function account_st_main($c = 0)
	{
		...
		if (count($this->children_ledgers) > 0)
		{
			...
			foreach ($this->children_ledgers as $id => $data)
			{
				...
				echo $this->print_space($this->counter);
				echo "&nbsp;" . anchor('report/ledgerst/' . $data['id'], $data['code'].'-'.$data['name'], array('title' => $data['name'] . ' Ledger Statement', 'style' => 'color:#000000'));
				echo "</td>";
