...
<body style="margin:0px;">
    ...
    ?></div><br><table class="cl"width="100%"border="1"cellpadding="0"cellspacing="0"style="margin-top: 45px;">
	<tr>
		<th align="left">&nbsp;</th>
	...
	if(isset($data)&&count($data))
	{
		...
		foreach ($data as $d)
		{
			...
			<th".($isthesame?" class='wdate'title='Change Withdrawal date'":'').">".
			($isthesame?$day:'')."<input class='dayheld' type='hidden' value='$day'/></th>
			<td>{$d['name']}</td>
			<td>{$d['paytype']}</td>
			<td>{$d['bankacctno']}</td>
			<td>{$d['bankbranch']}</td>
			...
			<td>{$d['balance']}</td>
			<td>{$d['amtodrawn']}</td>
			<td>".($isthesame?"<input class='sbtn'type='button'value='Submit'/>":'&nbsp;')."</td></tr>";
			$acctno = $d['CI_AcctNo'];

			unset( $disabled, $isthesame, $isColl, $qq );
